...
Value_t HashDesign::Get(Key_t& key) {
    auto f_hash = hash_funcs[0](&key, sizeof(Key_t), f_seed);
    auto f_idx = (f_hash & kMask) * kNumPairPerCacheLine;
...
double HashDesign::Utilization(void){
    size_t sum = 0;
    size_t cnt = 0;
...
size_t HashDesign::Capacity(void) {
    size_t cnt = 0;
...
// for debugging
Value_t HashDesign::FindAnyway(Key_t& key) {
    using namespace std;
